...
When you call the system functions to read directory entries, the OS typically returns the file name *and* its type (and on Windows, stat information such as file size and last modified time). However, the original Go and Python interfaces threw away this extra information, often requiring you to make an additional `stat()` call per entry -- and system calls aren't cheap.
...
In Python the function is `os.scandir(path)`, and it returns an iterator of `os.DirEntry` objects, which are as follows:
...
In Go the function is `os.ReadDir(path)`, and it returns a slice of `os.DirEntry` objects, which look like this:
